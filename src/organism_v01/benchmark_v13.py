...
V13_THRESHOLDS = {
    "reverse_target_set_accuracy": 0.55,
    "cycle_target_set_accuracy": 0.45,
    "reverse_slot_accuracy": 0.65,
    "cycle_slot_accuracy": 0.65,
    "reverse_routed_slot_accuracy": 0.80,
    "cycle_routed_slot_accuracy": 0.80,
    "normal_control_target_set_accuracy": 0.50,
    "erase_source_max_target_set_accuracy": 0.35,
    "erase_sink_max_target_peak_accuracy": 0.35,
    "swap_source_max_target_set_accuracy": 0.10,
    "erase_rule_balanced_max_target_set_accuracy": 0.55,
}


def summarize_v13_result(
    ...
    checks = {
        ...
        >= V13_THRESHOLDS["reverse_target_set_accuracy"],
        "cycle_target_set_accuracy": cycle["target_set_accuracy"]
        >= V13_THRESHOLDS["cycle_target_set_accuracy"],
        "reverse_slot_accuracy": reverse["slot_accuracy"] >= V13_THRESHOLDS["reverse_slot_accuracy"],
        "cycle_slot_accuracy": cycle["slot_accuracy"] >= V13_THRESHOLDS["cycle_slot_accuracy"],
        "reverse_routed_slot_accuracy": reverse["routed_slot_accuracy"]
        >= V13_THRESHOLDS["reverse_routed_slot_accuracy"],
        "cycle_routed_slot_accuracy": cycle["routed_slot_accuracy"]
        >= V13_THRESHOLDS["cycle_routed_slot_accuracy"],
        "normal_control_target_set_accuracy": controls["normal"]["target_set_accuracy"]
        >= V13_THRESHOLDS["normal_control_target_set_accuracy"],
        "erase_source_target_set_accuracy": controls["erase_source"]["target_set_accuracy"]
        <= V13_THRESHOLDS["erase_source_max_target_set_accuracy"],
        "erase_sink_target_peak_accuracy": controls["erase_sink"]["target_peak_accuracy"]
        <= V13_THRESHOLDS["erase_sink_max_target_peak_accuracy"],
        "swap_source_target_set_accuracy": controls["swap_source"]["target_set_accuracy"]
        <= V13_THRESHOLDS["swap_source_max_target_set_accuracy"],
        "erase_rule_balanced_target_set_accuracy": erase_rule_balanced
        <= V13_THRESHOLDS["erase_rule_balanced_max_target_set_accuracy"],
    }
    return {
        "passed": all(checks.values()),
        "checks": checks,
        "thresholds": V13_THRESHOLDS,
        "erase_rule_balanced_target_set_accuracy": erase_rule_balanced,
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the v0.13 damaged 3-pair survival benchmark.")
    ...
    parser.add_argument("--report", default="outputs/reports/benchmark-v13.json")
    return parser
...
def main() -> None:
    ...
    if checkpoint_args.get("update_rule") != "rank_slot_rule_cued":
        raise ValueError("v0.13 benchmark requires update_rule=rank_slot_rule_cued")
    if layout.rule_channels < 3:
        raise ValueError("v0.13 benchmark requires one-hot rule cue with rule_channels >= 3")
    ...
    report = {
        "model": args.model,
        "seed": args.seed,
        "benchmark": "v0.13_damaged_three_pair_survival",
        "config": {
            "batches": args.batches,
